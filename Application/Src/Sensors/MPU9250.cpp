...
void MPUXX50::setup(I2C_HandleTypeDef *pI2Cx, uint8_t addr)
{
	_pI2Cx = pI2Cx;
	_addr = addr << 1;
}
...
bool MPUXX50::readRawData(RawData& rawData)
{
    // Data out and buffer init
    uint8_t buf[14];

    // Subroutine for reading the raw data
    if (HAL_OK == HAL_I2C_Mem_Read(_pI2Cx, _addr, ACCEL_XOUT_H, 1, buf, 14, I2C_TIMOUT_MS))
	{
        // Bit shift the data
...
bool MPUXX50::processData(ProcessedData& processedData)
{
...
bool MPUXX50::calcAttitude(Attitude& attitude)
{
    // Read processed data
    ProcessedData sensorData;

    if (processData(sensorData))
    {
